...
namespace Ayubo_App.ChildForm
{
    public partial class Long_Tour : Form
    {
        ...
        private void cal()
        {
            ...
            int extrakm = endkm - startkm;
            int totalkm = dailykmL * date;
            if (extrakm > totalkm)
            {
                extraCharge = perkm * (extrakm - totalkm);
                txtExtrakmCharge.Text = Convert.ToString(extraCharge);
            }
            else
            {
            ...
            int OvernightCharge = Convert.ToInt32(txtOvernightCharge.Text);
            int extracharge = Convert.ToInt32(txtExtrakmCharge.Text);


            int total = Drivercost + hirevalue + Basehire + OvernightCharge + extracharge;
        ...
        private void comPackage_TextChanged(object sender, EventArgs e)
        {
        ...
        private void txtDrivercost_TextChanged(object sender, EventArgs e)
        {
            cal();
        }

        private void txtHireValue_TextChanged(object sender, EventArgs e)
        {
            cal();
        }

        private void txtBaseHireCharge_TextChanged(object sender, EventArgs e)
        {
            cal();
        }

        private void txtOvernightCharge_TextChanged(object sender, EventArgs e)
        {
            cal();
        }
        ...
        private void btnDTSave_Click(object sender, EventArgs e)
        {
            ...
            int LongHire = commands.generateid(commandid);
            ...
            commandinsert = "INSERT INTO Customer VALUES ('" + cuid + "','" + txtFName.Text + "', '" + txtLName.Text + "', '" + txtAddress.Text + "', '" + txtNIC.Text + "', '" + txtContact.Text + "')"
                + "INSERT INTO Booking VALUES ('" + txtBookingID.Text + "','" + cuid + "', '" + VehicleID + "')"
                 + "INSERT INTO Long_Tour VALUES ('" + LongHire + "','" + comPackage.Text + "', '" + dateTimePicker1.Value + "', '" + dateTimePicker2.Value + "','" + txtStartkm.Text + "', '" + txtEndkm.Text + "', '" + txtBaseHireCharge.Text + "','" + txtOvernightCharge.Text + "', '" + txtExtrakmCharge.Text + "', '" + txtHireValue.Text + "', '" + DriverID + "', '" + pacid + "', '" + txtBookingID.Text + "')"
        ...
        private void btnDTSearch_Click(object sender, EventArgs e)
        {
            ...
            search = "SELECT * FROM Long_Tour WHERE Booking_ID = '" + txtBookingID.Text + "'";
            values = commamds.search(search);
            comPackage.Text = values[1].ToString();
            dateTimePicker1.Text = values[2].ToString();
            dateTimePicker2.Text = values[3].ToString();
            txtStartkm.Text = values[4].ToString();
            txtEndkm.Text = values[5].ToString();
            txtBaseHireCharge.Text = values[6].ToString();
            txtOvernightCharge.Text = values[7].ToString();
            txtExtrakmCharge.Text = values[8].ToString();

            search = "SELECT * FROM Vehicle INNER JOIN Booking ON Booking.Vehical_ID = Vehicle.Vehicle_ID WHERE Booking.Booking_ID = '" + txtBookingID.Text + "'";
            values = commamds.search(search);
            comVehicleNo.Text = values[3].ToString();
            txtHireValue.Text = values[5].ToString();

            search = "SELECT First_Name FROM Long_Tour INNER JOIN Driver ON Driver.Driver_ID = Long_Tour.Driver_ID  WHERE Day_Tour.Booking_ID = '" + txtBookingID.Text + "'";
            values = commamds.search(search);
            comDriver.Text = values[1].ToString();
        }
        ...
        private void btnDTUpdate_Click(object sender, EventArgs e)
        {
            SQLCOMMANDS commamds = new SQLCOMMANDS();
            string update;
            update = "UPDATE Long_Tour set Package_type='" + comPackage.Text + "', Start_Date='" + dateTimePicker1.Value + "', End_Date= '" + dateTimePicker2.Value + "', Start_km= '" + txtStartkm.Text + "', End_Km= '" + txtEndkm.Text + "', Base_Hire_charge= '" + txtBaseHireCharge.Text + "', Overnight_stay_charge= '" + txtOvernightCharge.Text + "', Extra_km_Charge='" + txtExtrakmCharge.Text + "', Hire_Value='" + txtHireValue.Text + "', Driver_ID= '" + DriverID + "', Package_ID= '" + pacid + "' WHERE Booking_ID = '" + txtBookingID.Text + "')";
